...
/**
 * Sanitize a URL string: trim, strip wrapping quotes (and escaped quotes), ensure scheme, no trailing slash.
...
function sanitizeBaseUrl(value: string): string {
  ...
  // Strip escaped quotes from Vercel/env (e.g. \"https://...\" or "\"https://...\"")
  while (s.startsWith('\\"') || s.startsWith("\\'")) s = s.slice(2).trim()
  while (s.endsWith('"') || s.endsWith("'") || s.endsWith('\\"') || s.endsWith("\\'")) {
    if (s.endsWith('\\"') || s.endsWith("\\'")) s = s.slice(0, -2).trim()
    else s = s.slice(0, -1).trim()
  }
