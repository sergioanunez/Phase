...
export async function getBuildCycleTimeKpi(tenantId: string): Promise<BuildCycleTimeKpi> {
  ...
  for (const home of homes) {
    const taskStartedDates = home.tasks
      .map((t) => (t.startedAt ? new Date(t.startedAt) : null))
    ...
    const start =
      ...
      (taskStartedDates.length > 0 ? startOfDay(new Date(Math.min(...taskStartedDates.map((d) => d.getTime())))) : null) ??
    ...
    const completion =
      ...
      startOfDay((home as any).closedAt) ??
      (taskCompletedDates.length > 0 ? startOfDay(new Date(Math.max(...taskCompletedDates.map((d) => d.getTime())))) : null)
